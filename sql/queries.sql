SELECT scheme_name, fund_house, scheme_aum_crore FROM fact_performance
ORDER BY scheme_aum_crore DESC LIMIT 5;
...
SELECT dim_fund.scheme_name, dim_fund.fund_house, dim_fund.category, dim_fund.expense_ratio_pct, fact_performance.scheme_aum_crore
...
SELECT risk_category, COUNT(*) AS fund_count, SUM(scheme_aum_crore) AS total_scheme_aum_crore FROM dim_fund JOIN fact_performance ON dim_fund.amfi_code = fact_performance.amfi_code
GROUP BY risk_category ORDER BY total_scheme_aum_crore DESC;


SELECT dim_fund.fund_house, COUNT(*) AS scheme_count, ROUND(SUM(fact_performance.scheme_aum_crore), 2) AS total_scheme_aum_crore
FROM dim_fund JOIN fact_performance ON dim_fund.amfi_code = fact_performance.amfi_code GROUP BY dim_fund.fund_house
ORDER BY total_scheme_aum_crore DESC;