...
def create_database():
    ...
    try:
        with engine.begin() as conn:
            script = (PROJECT_ROOT / "sql" / "schema.sql").read_text(encoding="utf-8")
        ...
        raise RuntimeError("Database creation failed") from e

    return engine

def load_data(engine):
    funds = pd.read_csv(PROJECT_ROOT / "data" / "raw" / "01_fund_master.csv")
    nav = pd.read_csv(PROJECT_ROOT / "data" / "processed" / "02_nav_history_clean.csv")
    transactions = pd.read_csv(PROJECT_ROOT / "data" / "processed" / "08_investor_transactions_clean.csv")
    performance = pd.read_csv(PROJECT_ROOT / "data" / "processed" / "07_scheme_performance_clean.csv")
    aum = pd.read_csv(PROJECT_ROOT / "data" / "raw" / "03_aum_by_fund_house.csv")
    sip = pd.read_csv(PROJECT_ROOT / "data" / "raw" / "04_monthly_sip_inflows.csv")
    folio_count = pd.read_csv(PROJECT_ROOT / "data" / "raw" / "06_industry_folio_count.csv")
    category_inflows = pd.read_csv(PROJECT_ROOT / "data" / "raw" / "05_category_inflows.csv")
    benchmark = pd.read_csv(PROJECT_ROOT / "data" / "raw" / "10_benchmark_indices.csv")
    ...
    folio_count["month"] = pd.to_datetime(folio_count["month"]).dt.strftime("%Y-%m-%d")
    category_inflows["month"] = pd.to_datetime(category_inflows["month"]).dt.strftime("%Y-%m-%d")
    ...
    folio_count.to_sql("fact_folio_count", engine, if_exists="append", index=False)
    category_inflows.to_sql("fact_category_inflows", engine, if_exists="append", index=False)
    benchmark.to_sql("fact_benchmark", engine, if_exists="append", index=False)
